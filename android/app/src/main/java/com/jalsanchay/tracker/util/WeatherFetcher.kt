...
suspend fun fetchWeatherForLocation(
    ...
    try {
        ...
        if (!geoResponse.isSuccessful) return@withContext WeatherResult.Error("Location not found")
        val geoJson = JSONObject(geoResponse.body?.string().orEmpty())
        val first = geoJson.optJSONArray("results")?.optJSONObject(0)
            ?: return@withContext WeatherResult.Error("Location not found")
        ...
        if (!forecastResponse.isSuccessful) return@withContext WeatherResult.Error("Forecast unavailable")
